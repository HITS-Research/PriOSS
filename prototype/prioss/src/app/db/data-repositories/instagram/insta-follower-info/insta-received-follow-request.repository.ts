...
export class InstaReceivedFollowRequestRepository extends BulkAddCapableRepository {
     ...
     * @author: Melina (kleber@mail.uni-paderborn.de)
     */
    async startReceivedFollowRequestBulkAdd(instaProfileURL: string, instaAccountName: string, timestamp: number, totalRowCount: number, targetBulkSize: number = 500) {
    ...
    async addReceivedFollowRequestInformation(instaProfileURL: string, instaAccountName: string, timestamp: number) {
        await this.dbService.executeQuery<any>(async (db: SQLiteDBConnection) => {
            ...
            let sqlStatement = sql.insertReceivedFollowRequestSQL;
    ...
    async getReceivedFollowRequestInfo(): Promise<InstaReceivedFollowRequestInfo[]>
    {
        return this.dbService.executeQuery<any>(async (db: SQLiteDBConnection) => {
            ...
            let result = await db.query(sql.selectReceivedFollowRequestInfo);

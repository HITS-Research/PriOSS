...
export class InstaRecentFollowRepository extends BulkAddCapableRepository {
     ...
     * @author: Melina (kleber@mail.uni-paderborn.de)
     */
    async startRecentFollowBulkAdd(instaProfileURL: string, instaAccountName: string, timestamp: number, totalRowCount: number, targetBulkSize: number = 500) {
    ...
    async addRecentFollowInformation(instaProfileURL: string, instaAccountName: string, timestamp: number) {
        await this.dbService.executeQuery<any>(async (db: SQLiteDBConnection) => {
            ...
            let sqlStatement = sql.insertIntoInstaRecentFollowSQL;
    ...
    async getRecentFollowInfo(): Promise<InstaRecentFollowInfo[]>
    {
        return this.dbService.executeQuery<any>(async (db: SQLiteDBConnection) => {
            ...
            let result = await db.query(sql.selectRecentFollowInfo);
